  ...
  return (
    ...
    >
      <div ref={ref} className="w-full max-w-6xl">
        <SectionHeader title="Projects" subtitle="Selected work from the last few years." />
        <motion.div {...staggerProps} className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {PROJECTS.map(project => (
            <motion.div key={project.title} variants={FADE_UP_VARIANTS} className="h-full">
              <Card className="flex h-full flex-col">
                <div className="mb-2 flex items-start justify-between gap-2">
                  ...
                  {project.liveUrl && <IconLink icon="↗" href={project.liveUrl} external />}
                </div>
